...
def create_all_pii(infiles = ["/content/drive/Shareddrives/BigScience/pii_annotated/reviewed/*", "/content/drive/Shareddrives/BigScience/pii_annotated/annotated/pii_*"], outfile="all_pii.jsonl"):
  seen = {}
        ...
        if annotations or dlist[i]['metadata'].get('ner'):
          ...
          for entity_tag in ner:
            ...
            if entity in text:
              ...
              idx = (start, end, entity)
              aHash = ner2.get(idx, {})
              aHash[tag] = aHash.get(tag, 0) +  2.0
              ner2[idx]  = aHash
          for a in annotations:
            tag = a['tag']
            idx = (a['start'], a['end'], a['value'])
            aHash = ner2.get(idx, {})
